...
const TYPE_ORDER = { directory: 0, file: 1, other: 2 };
    ...
    ctx.tools.register(defineTool({
        ...
        output: {
            ...
            render: (_args, value) => {
                ...
                // Truncation notice
                if (value.truncated) {
                    parts.push(`[${value.truncated.remaining} items truncated, showing first ${value.truncated.shown} of ${value.truncated.total} total]`, '');
                }
                // Statistics summary
                parts.push(`Total: ${value.stats.total} entries ` +
        ...
        async execute(args, exec) {
            ...
            const target = await ctx.fs.resolve(args.path, cwd === undefined ? { signal: exec.signal } : { cwd, signal: exec.signal });
